...
const Utils = {
    // 格式化金�?    formatMoney(amount, showSign = false) {
    ...
    // 格式化日�?    formatDate(dateStr, format = 'short') {
        if (!dateStr) return '-';
    ...
    // 格式化相对时�?    formatRelativeTime(dateStr) {
        if (!dateStr) return '';
        const date = new Date(dateStr);
        const now = new Date();
        const diff = date - now;
...
// 成本类型配置
const COST_TYPES = [
    { value: 'labor', label: '人工分包�?, icon: '👷' },
    { value: 'material', label: '设备材料�?, icon: '📦' },
...
// 项目状态配�?const PROJECT_STATUS = [
    { value: '进行�?, color: '#1890ff' },
    { value: '已完�?, color: '#52c41a' },
    { value: '已暂�?, color: '#faad14' },
    { value: '已取�?, color: '#ff4d4f' }
];

// 付款方式配置
const PAYMENT_TYPES = [
    { value: 'progress', label: '进度�?, desc: '按工程进度支�? },
    { value: 'milestone', label: '节点�?, desc: '完成特定节点支付' },
    { value: 'final', label: '尾款', desc: '完工验收后支�? },
